...
void HeaterTestScreen::update(LiquidCrystalSerial& lcd, bool forceRedraw) {
	...
	if(heater_timeout.hasElapsed())
	{
		...
		{
			lcd.setCursor(0,0);
			lcd.writeFromPgmspace(SPLASH1A_MSG);
			
			lcd.setCursor(0,1);
			lcd.writeFromPgmspace(SPLASH4A_MSG);

			lcd.setCursor(0,2);
			lcd.writeFromPgmspace(SPLASH3A_MSG);
		}
		else
		{
			lcd.setCursor(0,0);
			lcd.writeFromPgmspace(SPLASH2A_MSG);
			
			lcd.setCursor(0,1);
			lcd.writeFromPgmspace(SPLASH5A_MSG);
		...
		lcd.setCursor(0,3);
		lcd.writeFromPgmspace(SPLASH2B_MSG);
	}

	if (forceRedraw) {
		lcd.setCursor(0,0);
		lcd.writeFromPgmspace(SPLASH1B_MSG);

		lcd.setCursor(0,1);
		lcd.writeFromPgmspace(SPLASH2B_MSG);

		lcd.setCursor(0,2);
		lcd.writeFromPgmspace(SPLASH3B_MSG);

		lcd.setCursor(0,3);
		lcd.writeFromPgmspace(SPLASH4B_MSG);
	}
}
...
void JogMode::update(LiquidCrystalSerial& lcd, bool forceRedraw) {
	...
	/*	switch (jogDistance) {
		case DISTANCE_SHORT:
			lcd.writeFromPgmspace(DISTANCESHORT_MSG);
			break;
		case DISTANCE_LONG:
			lcd.writeFromPgmspace(DISTANCELONG_MSG);
			break;
		}
   */
...
void PreheatSettingsMenu::drawItem(uint8_t index, LiquidCrystalSerial& lcd) {
	...
	switch (index) {
        case 0:
            lcd.writeFromPgmspace(SET1_MSG);
            break;
        case 1:
            if(!singleTool){
...
void ResetSettingsMenu::drawItem(uint8_t index, LiquidCrystalSerial& lcd) {
	...
	switch (index) {
        case 0:
            ...
            break;
        case 1:
            lcd.writeFromPgmspace(SET2_MSG);
            break;
        case 2:
            lcd.writeFromPgmspace(NO_MSG);
            break;
