...
void SettingsMenu::resetState(){
    ...
    uint8_t LEDSetting = (eeprom::getEeprom8(eeprom_offsets::LED_STRIP_SETTINGS, 1)) ;
    int Idx = 0;
    while(LEDSetting >> Idx != 1)
      Idx++;
    LEDColor = Idx;
...
void SettingsMenu::handleSelect(uint8_t index) {
	switch (index) {
		...
		case 3:
            ...
            if(LEDColor > 6)
                LEDColor = 0;
            eeprom_write_byte((uint8_t*)eeprom_offsets::LED_STRIP_SETTINGS, (1 << LEDColor));
