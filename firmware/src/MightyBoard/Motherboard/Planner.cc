...
namespace planner {
	...
	void init()
	{
		...
		if (accelerationStatus &  _BV(ACCELERATION_INIT_BIT)  == 0){
		...
		setMaxAxisJerk(eeprom::getEepromFixed16(eeprom_offsets::ACCELERATION_SETTINGS + acceleration_eeprom_offsets::AXIS_JERK_OFFSET + 2, DEFAULT_MAX_Z_JERK), 2);
