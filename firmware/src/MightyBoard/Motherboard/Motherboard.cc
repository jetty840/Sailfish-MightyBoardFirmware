...
Motherboard::Motherboard()
    :
      ...
      platform_heater(
          ...
          eeprom_offsets::T0_DATA_BASE + toolhead_eeprom_offsets::HBP_PID_BASE,
          false, 2),
      using_platform(eeprom::getEeprom8(eeprom_offsets::HBP_PRESENT, 1)),
#ifdef MODEL_REPLICATOR2
      Extruder_One(0, EXA_PWR, EXA_FAN, ThermocoupleReader::CHANNEL_ONE,
                   eeprom_offsets::T0_DATA_BASE),
      Extruder_Two(1, EXB_PWR, EXB_FAN, ThermocoupleReader::CHANNEL_TWO,
                   eeprom_offsets::T1_DATA_BASE)
#else
...
// Piezo::shutdown_timer();
  ...
  // Timer 5 is 16 bit
  //
  //   - Microsecond timer, SD card check timer, P-Stop check timer, LED
  // flashing timer
...
void Motherboard::reset(bool hard_reset) {
  ...
  // do not clear heater fail messages, though the user should not be able to
  // soft reboot from heater fail
  if (hard_reset) {
...
micros_t Motherboard::getCurrentMicros() {
  ...
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) { micros_snapshot = micros; }
...
void Motherboard::doStepperInterrupt() {
  ...
  if (STEPPER_TCNTn >= STEPPER_OCRnA) {
    STEPPER_OCRnA = 0x01; // We set the next interrupt to 1 interval, because
                          // this will cause the
...
void Motherboard::HeatingAlerts() {
  ...
  // TODO: top temp should use preheat temps stored in eeprom instead of a hard
  // coded value
  ...
  if (heater0.isHeating() || heater1.isHeating() ||
    ...
    if ((div_temp != 0) && eeprom::heatLights()) {
      ...
      RGB_LED::setColor((255 * abs((setTemp - deltaTemp))) / div_temp, 0,
...
// MBI's code has a design flaw whereby it manages all heaters, whether they
// exist or
// not and even if they are disabled!  Thus, the Heater::manage_temperature()
// routine
// will attempt termperature reads on disabled heaters.  That, in turn, leads to
// failures
// on non-existent or otherwise disabled heaters.  And when they fail, this
// routine is called.
// Thus this routine ends up having to decide whether a failure is a false alarm
// or not.
// The logic for doing that is non-trivial.  But, more importantly, such logic
// should not
// have to exist: the manage routine shouldn't be trying to manage non-existent
// or disabled
// heaters!

// It would seem that some of the code in this routine is the non-trivial logic
// trying to
// figure out whether or not a heater error should be ignored.  Thus, some of
// the code
// here is a work around to the deeper problem.  Worse yet, there's an "if" test
// below
// which is simply incorrect: it allows a Rep 2 (single heater) to keep on
// running when
// its single heater fails with a "not plugged in" error:
//
//    !platform_heater.has_failed() == !false == true  // no heated platform on
// Rep 2
//    eeprom::isSingleTool() == true                   // Rep 2 is single tool
//    !(Extruder_One...has_failed() && Extruder_Two...has_failed()) == !(true &&
// false) == !(false) == true
//      ^^^ By the above, BOTH heaters have to fail on a Rep 2, but a Rep 2 has
// only one heater
//
// Net result of the above logic is to always ignore a "not plugged in" error on
// a Rep 2 *unless*
// the management routines just happen to also trigger an error on the
// non-existent HBP or
// non-existent 2nd extruder.
  ...
  heatShutdown =
      slave_id + 1; // slave_ids are 0 = tool 0, 1 = tool 1, 2 = platform
}
...
void Motherboard::runMotherboardSlice() {
  ...
  if (hasInterfaceBoard) {
    ...
    // stagger motherboard updates so that they do not all occur on the same
    // loop
  ...
  // ( in this case heaters are already shutdown and separate error messaging
  // used)
  ...
  if (heatShutdown && !triggered && !Piezo::isPlaying()) {
    ...
    if (heatShutdown < 3) {
      if (eeprom::isSingleTool())
        msg = HEATER_TOOL_MSG;
      else if (heatShutdown == 1)
        msg = HEATER_TOOL0_MSG;
      else
        msg = HEATER_TOOL1_MSG;
    } else
      msg = HEATER_PLATFORM_MSG;
...
ISR(STEPPER_TIMERn_COMPA_vect) { Motherboard::getBoard().doStepperInterrupt(); }

#if defined(PSTOP_SUPPORT) && defined(PSTOP_VECT)

ISR(PSTOP_VECT) {
  if ((Motherboard::getBoard().pstop_enabled == 1) &&
      (PSTOP_PORT.getValue() == 0))
...
// if both times are zero, LEDs are full on, if just on-time is zero, LEDs are
// full OFF
...
ISR(TIMER2_COMPA_vect) { steppers::doExtruderInterrupt(); }
#endif
...
ISR(TIMER5_COMPA_vect) {
  ...
  if (SD_DETECT_PIN.getValue() != 0x00)
...
void Motherboard::setUsingPlatform(bool is_using) { using_platform = is_using; }
...
void BuildPlatformHeatingElement::setHeatingElement(uint8_t value) {
  ...
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) { HBP_HEAT.setValue(value != 0); }
}
...
void Motherboard::heatersOff(bool platform) {
  ...
  if (platform)
    motherboard.getPlatformHeater().set_target_temperature(0);
  BOARD_STATUS_CLEAR(Motherboard::STATUS_PREHEATING);
}

void Motherboard::interfaceBlinkOn() { motherboard.interfaceBlink(25, 15); }

void Motherboard::interfaceBlinkOff() { motherboard.interfaceBlink(0, 0); }
...
uint8_t Motherboard::getCurrentError() { return blink_count; }
