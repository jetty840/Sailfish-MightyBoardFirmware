...
wget -N -P incoming http://ftp.gnu.org/gnu/gcc/gcc-${GCC_VERSION}/gcc-g++-${GCC_VERSION}.tar.bz2
	...
	pv incoming/gcc-core-${GCC_VERSION}.tar.bz2 | bzip2 -dc | tar xf -
	pv incoming/gcc-g++-${GCC_VERSION}.tar.bz2 | bzip2 -dc | tar xf -
	
	# patch to build avr-gcc-4.6.2 with gcc-4.7
	if test -f incoming/avr-gcc-${GCC_VERSION}.patch
	then
		cd gcc-${GCC_VERSION}
		patch -p1 < ../incoming/avr-gcc-${GCC_VERSION}.patch
		cd ..
	fi
